...
set _DeploySetParametersFile=%RootPath%icoop_webapp.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%icoop_webapp.zip" (
echo "%RootPath%icoop_webapp.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%icoop_webapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%icoop_webapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%icoop_webapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%icoop_webapp.SetParameters.xml" %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%icoop_webapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%icoop_webapp.SetParameters.xml" %_MsDeployAdditionalFlags%
)
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%icoop_webapp.deploy-readme.txt" (
...
start notepad "%RootPath%icoop_webapp.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%icoop_webapp.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%icoop_webapp.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
